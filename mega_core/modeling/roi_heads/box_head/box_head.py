...
class DynamicHead(nn.Module):
    ...
    def _init_box_pooler(cfg, input_shape):
        # 初始化 ROIPooler 对象，用于在模型中进行 ROI pooling 操作
    ...
    def forward(self, features, init_bboxes, t, init_features, box_extract=0):
        ...
        time = self.time_mlp(t)  # 通过 MLP 处理时间步信息，获取时间表示

        inter_class_logits = []   # 存储中间步骤的分类 logits
        inter_pred_bboxes = []  # 存储中间步骤的预测边界框

        bs = len(features[0])  # 获取 batch size
        dim = features[0].size(1)   # 获取特征的维度
        bboxes = init_bboxes   # 噪声框
        num_boxes = bboxes.shape[1]  # 噪声框数量
        ...
        if self.training or box_extract > 0 or self.sampling_timesteps > 1:
            ...
            for head_idx, rcnn_head in enumerate(self.head_series):
                ...
                bboxes = pred_bboxes.detach()  # 防止梯度回传到之前的rcnn_head，以确保每个头部独立地更新梯度。
        elif box_extract == 0:
            # skip 0~2 stage
        ...
        if self.local_enable or self.global_enable:
            ...
            # global box-level attention  遍历全局注意力机制列表
            ...
            # 通过自适应归一化的方式，对局部注意力权重进行了处理，其中训练阶段对一定比例的注意力权重进行置零，以实现对类别嵌入的控制。
            if self.adaptive_norm:
            ...
            for head_idx, rcnn_head_local in enumerate(self.head_series_cond):
                ...
                if self.return_intermediate:  # 是否添加中间结果
        ...
        if self.return_intermediate and self.training:
            if self.local_enable:  # 将中间结果堆叠起来返回
...
class RCNNHead(nn.Module):
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        assert not torch.isnan(deltas).any(), "deltas contains NaN values"
        ...
        pred_boxes[:, 0::4] = pred_ctr_x - 0.5 * pred_w  # x1  # 双冒号表示从索引0开始，步长为4切片。实际是start:end:step省略了
        pred_boxes[:, 1::4] = pred_ctr_y - 0.5 * pred_h  # y1
        pred_boxes[:, 2::4] = pred_ctr_x + 0.5 * pred_w  # x2
        pred_boxes[:, 3::4] = pred_ctr_y + 0.5 * pred_h  # y2
        # 下面断言的作用：保证右下角坐标大于左上角坐标，因为nan值可能会出现断言错误，处理一下。
